...
SELECT
    ...
    rua.user_id AS assigned_user_id
FROM referral r
...
COMMENT ON COLUMN case_list_view.assigned_user_id IS 'Foreign key reference to the assigned user (null if unassigned)';
...
CREATE INDEX idx_case_list_view_assigned_user ON case_list_view (assigned_user_id);
