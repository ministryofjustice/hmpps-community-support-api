...
class RiskInformationService(
  ...
  fun saveDraftRiskInformation(
    ...
    val riskInformation = existing?.apply {
      ...
      riskSummaryWhoIsAtRisk = request.riskSummaryWhoIsAtRisk.takeIf{ it?.isNotEmpty() ?: false },
      riskSummaryNatureOfRisk = request.riskSummaryNatureOfRisk.takeIf{ it?.isNotEmpty() ?: false },
      riskSummaryRiskImminence = request.riskSummaryRiskImminence.takeIf{ it?.isNotEmpty() ?: false },
      riskToSelfSuicide = request.riskToSelfSuicide.takeIf{ it?.isNotEmpty() ?: false },
      riskToSelfHarm = request.riskToSelfSelfHarm.takeIf{ it?.isNotEmpty() ?: false },
      riskToSelfHostelSetting = request.riskToSelfHostelSetting.takeIf{ it?.isNotEmpty() ?: false },
      riskToSelfVulnerability = request.riskToSelfVulnerability.takeIf{ it?.isNotEmpty() ?: false },
      additionalInformation = request.additionalInformation.takeIf{ it?.isNotEmpty() ?: false },
      updatedAt = OffsetDateTime.now(),
      updatedBy = userId,
      referral = referral,
    )
